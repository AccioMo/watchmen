        ...
        return (
            <div className="min-h-screen bg-[#0d0d0d] text-white pt-24 px-6 md:px-12 flex flex-col items-center justify-center text-center">
                ...
                <p className="text-white/60 mb-8 max-w-md">
                    Movies and TV shows you add to your watchlist will appear here. Browse to find something to watch!
                </p>
            ...
            <div className="container mx-auto">
                ...
                <div className="space-y-4">
                    {watchlist.map((item, index) => {
                        ...
                        const watchLink = isMovie ? `/movie/watch/${item.id}` : `/tv/watch/${item.id}`;

                        return (
                            <div key={`${item.media_type}-${item.id}`} className="bg-white/5 border border-white/5 rounded-xl p-4 flex gap-4 md:gap-6 items-center group hover:bg-white/10 transition-colors">
                                ...
                                {/* Card (Mini) */}
                                <div className="w-16 md:w-24 aspect-[2/3] flex-shrink-0 relative">
                                    <Link href={link} className="block w-full h-full overflow-hidden rounded-lg relative">
                                        <LazyImage
                                    ...
                                    <div className="absolute top-1 left-1 bg-black/60 backdrop-blur-md text-[10px] font-bold px-1.5 py-0.5 rounded text-white/90 uppercase tracking-wider border border-white/10">
                                        {item.media_type}
                                    </div>
                                </div>
                                ...
                                <div className="flex-1 min-w-0">
                                    <h3 className="text-lg md:text-xl font-bold text-white mb-1 truncate">{title}</h3>
