...
function Recommended() {
	...
	useEffect(() => {
		if (loading || recommendedMovies.length > 0) return;
		...
		const fetchMovies = async () => {
			try {
				...
				for (let i = 1; i < 4; i++) {
					promises.push(getTMDBMoviesBy("recommended", i));
				}
		...
		<div className="min-h-screen bg-gradient-to-br from-primary via-secondary to-accent animated-bg">
			<div className="bg-gradient-to-b from-transparent to-black/20">
					...
					{recommendedMovies && recommendedMovies[1] && (
						...
						>
							<picture className="opacity-95 -z-10">
								<source
									srcSet={getImageURL(
										recommendedMovies[1].backdrop_path,
										"max"
									)}
									sizes="100vh"
				...
				<div className="py-4">
					<div>
						<div className="mb-8">
							<TwoEmblaCarousel slides={[...recommendedMovies.slice(0, 15)]} direction="forward" options={{ loop: true, dragFree: true }} />
						</div>
						<div className="mb-8">
							<TwoEmblaCarousel slides={[...recommendedMovies.slice(15, 30)]} direction="backward" options={{ loop: true, dragFree: true }} />
						</div>
						<div className="mb-8">
							<TwoEmblaCarousel slides={[...recommendedMovies.slice(30, 45)]} direction="forward" options={{ loop: true, dragFree: true }} />
						</div>
					</div>
				</div>
			</div>
